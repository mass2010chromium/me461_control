...
void MPU9250_write_async(MPU9250* imu) {
    ...
    SPI_write(spi, imu->chipselect, packet_size, imu->packets_head, imu);
}
...
void MPU9250_write_blocking(MPU9250* imu) {
    ...
    uint16_t* packets_head = imu->packets;
    uint16_t* packet_sizes = imu->packet_sizes;
    for (; *packet_sizes; ++packet_sizes) {
        uint16_t packet_size = *packet_sizes;

        SPI_write(spi, imu->chipselect, packet_size, packets_head, 0);
...
void MPU9250_set_read_accel_gyro(MPU9250* imu) {
    ...
    if (imu->IO_mode == MPU9250_ASYNC) {
        SPI_write(imu->channel, imu->chipselect, 8, imu->packets_head, imu);
    }
    else {
