...
void setPWMA(PWM* pwm, float control_effort) {
    ...
    control_effort = (control_effort < 0) ? 0 : control_effort;
    control_effort = (control_effort > control_scale) ? control_scale : control_effort;
    control_effort = (control_effort * pwm->control_scale_inv * output_scale
            + pwm->output_min) * (*pwm->prd);
    pwm->regs->CMPA.bit.CMPA = control_effort;
}
...
void setPWMB(PWM* pwm, float control_effort) {
    ...
    control_effort = (control_effort * pwm->control_scale_inv * output_scale
            + pwm->output_min) * (*pwm->prd);
    pwm->regs->CMPB.bit.CMPB = control_effort;
}
