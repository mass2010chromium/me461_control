 ...
 ***************************************************************************/
char serial_printf_buf[BUF_SIZE];
...
uint16_t serial_printf(serial_t *s, char *fmt, ...)
{
    ...
    va_start(ap,fmt);
    vsprintf(serial_printf_buf,fmt,ap);
    va_end(ap);

    return serial_send(s,serial_printf_buf,strlen(serial_printf_buf));
}

//For Text LCD
//char UART_printf_buffer[BUF_SIZE];
